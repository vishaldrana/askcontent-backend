...
def test_a_forbidden_document_is_never_cited(platform):
    ...
    _, evidence = ask(platform, "cn-finance", "revenue recognition control narrative", "user:asha")
    assert evidence.trace.forbidden_count >= 1
    assert all(c.doc_id != "FIN-3300" for c in evidence.citations)
...
def test_a_forbidden_document_is_dropped_before_ranking(platform):
    ...
    _, evidence = ask(platform, "cn-finance", "revenue recognition control narrative", "user:asha")
    forbidden = next(c for c in evidence.trace.candidates if c.doc_id == "FIN-3300")
...
def test_a_stale_index_entry_is_counted_not_hidden(platform):
    ...
    _, evidence = ask(platform, "cn-platform-eng", "certificate rotation runbook", "user:rob")
    assert evidence.trace.stale_index_count >= 1
    stale = next(c for c in evidence.trace.candidates if c.doc_id == "RB-777")
...
def test_narrowing_scope_takes_effect_without_an_ingest_run(platform):
    ...
    connector, before = ask(platform, "cn-people-ops", "parental leave entitlement", "user:asha")
    assert any(c.doc_id == "HR-1041" for c in before.citations)

    narrowed = KnowledgeScope(
        roots=(SourceRoot(kind="space", value="HR"),), exclude=("/hr/policies/*",)
    )
    platform.registry.update_scope(connector.connector_id, narrowed, "admin", {})

    _, after = ask(platform, "cn-people-ops", "parental leave entitlement", "user:asha")
    assert all(c.doc_id != "HR-1041" for c in after.citations)
...
def test_suspension_takes_effect_on_the_next_query(platform):
    ...
    platform.registry.set_state("cn-people-ops", ConnectorState.SUSPENDED, "admin")
    _, evidence = ask(platform, "cn-people-ops", "parental leave", "user:asha")
...
def test_a_document_is_never_in_conflict_with_itself(platform):
    ...
    _, evidence = ask(platform, "cn-people-ops", "parental leave weeks", "user:asha")
...
def test_both_channels_run_and_a_failure_degrades_visibly(platform):
    ...
    _, evidence = ask(platform, "cn-people-ops", "expenses submission deadline", "user:asha")
    assert {t.channel for t in evidence.trace.channels} == {"pgp", "ecm"}

    failing = build(simulate_latency=False, failure_rate=1.0)
    _, degraded = ask(failing, "cn-people-ops", "expenses", "user:asha")
...
def test_every_drop_is_attributed_to_one_named_rule(platform):
    ...
    _, evidence = ask(platform, "cn-people-ops", "expenses policy limits", "user:asha")
...
def test_the_passage_cache_prevents_reparsing(platform):
    ...
    ask(platform, "cn-people-ops", "parental leave entitlement", "user:asha")
